...
function PointOfSale() {
  ...
  let date = `${fechaD.getFullYear()}-${
    parseInt(fechaD.getMonth()) + 1 < 10
      ? "0" + (parseInt(fechaD.getMonth()) + 1)
      : parseInt(fechaD.getMonth()) + 1
  }-${
    parseInt(fechaD.getDate()) + 1 < 10
      ? "0" + fechaD.getDate()
      : fechaD.getDate()
  }`;
  let time = `T${fechaD.getHours()}:${fechaD.getMinutes()}:${fechaD.getSeconds()}`;
  ...
  const handlerOrder = () => {
    console.log("Factura a realizar: ", factura);
